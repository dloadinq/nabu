...
import { BACKENDS, VISUALIZER_ID, CUSTOM_EVENTS } from '../core/constants.js';
import { appState, stateMachine, timestamp, setActiveBackend, emitStatusMessage, emitBackendChanged } from '../core/appState.js';
import { probeServiceBackend, disconnectService } from '../service/serviceClient.js';
import { stopAudioCapture } from '../audio/audioCapture.js';
import { stopWakeWordEngine, disposeWakeWordEngine } from '../audio/wakeWordEngine.js';
import { stopSpeechDetection, disposeSpeechDetector } from '../audio/speechDetector.js';
import { stopVisualizer, disposeVisualizer } from '../ui/audioVisualizer.js';
import { initWorkerClient, loadModelInWorker, disposeWorkerClient } from '../worker/workerClient.js';
...
export function startHeartbeat() {
    ...
    console.log(`[${timestamp()}] Heartbeat started.`);
}
...
export function stopHeartbeat() {
    if (heartbeatIntervalId) {
        ...
        console.log(`[${timestamp()}] Heartbeat stopped.`);
    }
}
    ...
    heartbeatIntervalId = setTimeout(async () => {
        ...
        try {
            ...
            if (!alive && appState.activeBackend === BACKENDS.SERVICE) {
                console.warn(`[${timestamp()}] Server heartbeat failed. Switching to WebGPU.`);
                ...
                console.log(`[${timestamp()}] Server detected. Auto-switching from browser to server.`);
...
export async function fallbackToBrowser() {
    ...
    if (_onRestartNeeded) {
        try { await _onRestartNeeded(); } catch (error) {
